...
-- Задание Урок 3. Вебинар. Введение в проектирование БД
-- Добавить необходимую таблицу/таблицы для того, чтобы можно было использовать лайки для медиафайлов, постов и пользователей.

-- Создаём таблицу типа лайков
CREATE TABLE like_type (
	id SERIAL PRIMARY KEY,
	name VARCHAR(200) NOT NULL UNIQUE
	);

-- добавляем типы лайков, которые может оставить пользователь
INSERT INTO like_type VALUES (DEFAULT, 'positive like');
INSERT INTO like_type VALUES (DEFAULT, 'negative like');

-- Создаём таблицу объектов, на которые можно проставить лайк
CREATE TABLE like_object (
	id SERIAL PRIMARY KEY,
	name VARCHAR(200) NOT NULL UNIQUE
);

-- добавляем типы объектов, которые могут "получить" лайк
INSERT INTO like_object VALUES (DEFAULT, 'медиафайл');
INSERT INTO like_object VALUES (DEFAULT, 'пост');
INSERT INTO like_object VALUES (DEFAULT, 'пользователь');

-- Создаём таблицу для лайков
CREATE TABLE likes (
	id SERIAL PRIMARY KEY,
	from_user_id BIGINT UNSIGNED NOT NULL,
	to_user_id BIGINT UNSIGNED NOT NULL,
	like_object_id BIGINT UNSIGNED NOT NULL,
	...
	CONSTRAINT fk_likes_like_object FOREIGN KEY (like_object_id) REFERENCES like_object (id),
	CONSTRAINT fk_likes_like_type FOREIGN KEY (like_type_id) REFERENCES like_type (id),
  	INDEX likes_like_object_idx (like_object_id),
  	INDEX likes_like_type_idx (like_type_id)
  );
